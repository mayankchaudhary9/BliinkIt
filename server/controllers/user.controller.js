...
import genaratedOtp from "../utils/generatedOtp.js";
...
export async function registerUserController(req, res) {
  try {
    ...
    const verfiyEmail = await sendEmail({
      sendTo: email,
      subject: "verify email from bliinkit",
...
// upload user avatar
export async function uplaodAvatar(req, res) {
  try {
...
export async function forgotPasswordController(req, res) {
  try {
    ...
    const otp = genaratedOtp();
    const expireTime = new Date() + 60 * 60 * 1000; // in 1 hr after generated
    ...
    await sendEmail({
      sendTo: email,
      subject: "Forgot password from Bliinkit.",
...
export async function verifyForgotPasswordOtp(req, res) {
  try {
    ...
    // now if otp is not expired and valid then
